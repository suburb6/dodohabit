...
const Navbar = () => {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex items-center justify-between border-b border-white/5 bg-[#0A0A0A]/70 backdrop-blur-md">
            {/* Logo */}
            <Link to="/" className="flex items-center gap-2 z-50">
                <img src="/icon.png" alt="DodoHabit" className="w-8 h-8 rounded-lg" />
                <span className="text-xl font-bold tracking-tight text-white hiding-sm">Dodohabit</span>
            </Link>

            {/* Desktop Navigation - Absolute Centered */}
            <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center bg-white/5 p-1 rounded-full border border-white/5 backdrop-blur-sm">
                                ...
                                {tabs.map((tab) => {
                                    ...
                                    return (
                                        <Link
                                            key={tab.id}
                                            to={tab.id}
                                            className={`text-xl font-bold py-2 ${isActive ? 'text-white' : 'text-gray-500'
                                                }`}
                                        >
                                            {tab.label}
                                        </Link>
